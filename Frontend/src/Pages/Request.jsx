...
function Request() {
    ...
    return [...requests].reverse().map((request, index) => (
      <div key={index} className="flex bg-white w-full sm:w-2/3 mx-auto shadow-lg rounded-lg p-6 mb-6 transition-transform transform hover:scale-105 hover:shadow-xl">
        ...
        <img 
          ...
          className="w-24 h-24 object-cover rounded-md border border-gray-200 mr-6"
        />
    
        <div className="flex-1">
          <h3 className="text-xl font-semibold text-gray-800 hover:text-blue-500 transition-colors mb-2 cursor-pointer" onClick={() => (navigate(`/property/${request.property._id}`))}>
  ...
  return (
    <div className="lg:ml-64 mt-6">
      <div className="h-12 w-2/3 mx-auto bg-gray-100 rounded-lg shadow-md flex">
        {/* Sent Tab */}
        <div
